...
class Ad
{
    ...
    public function initiliazeSlug(): void {
        if(empty($this->slug)) {
            $slugiy = new Slugify();
            $this->slug = $slugiy->slugify($this->title);
        }
    }
    ...
    public function getAvgRatings() {
        ...
        // Ici comments est un ArrayCollection d'où la nécessité de le transformer en Array classique
        $sum = array_reduce($this->comments->toArray(), function($total, $element) {
            return $total + $element->getRating();
        },0);
    ...
    /**
     * Permet d'obtenir un tableau des jours qui ne sont pas disponibles pour cet appartement
     *
