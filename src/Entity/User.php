...
class User implements UserInterface
{
    ...
    /**
     * @ORM\ManyToMany(targetEntity="App\Entity\Role", mappedBy="users", orphanRemoval=true)
     */
    private $userRoles;
     ...
     * @return void
     */
    public function initiliazeSlug(): void {
        if(empty($this->slug)) {
            $slugiy = new Slugify();
            $this->slug = $slugiy->slugify($this->firstName . '-' . $this->lastName);
        }
    }
    ...
    // Cette méthode est appelée par le composant de sécurité pour l'accés aux pages, au moment de la connexion de l'utilisateur
    public function getRoles() {
        // Map est une méthode spécifique aux ArrayCollection et qui permet de récupérer l'élement itérant via un callback, pour ensuite créer un nouveau tableau (avec comme élément ce que je décris dans le "return")
        $roles = $this->userRoles->map(function($role) {
            return $role->getTitle();
        });
        // Transformation d'un tableau de type ArrayCollection en un tableau plus "classique PHP"
        $roles = $roles->toArray();
        $roles[] = 'ROLE_USER';
        return $roles;
    }
